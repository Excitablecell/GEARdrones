...
uint8_t uart_rxdata[200] = {0},data_size;
uint8_t optic_flow_data[18] = {0},flow_size;
uint8_t uwb_data[400] = {0};
size_t uwb_size;
	...
	if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET)
		{
			...
			flow_size = 18 - __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);
			
//			memset(optic_flow_data,0,18);不能memset 除非使用队列传进其他对象，否则容易在读取数据途中被0覆盖
	...
	if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_IDLE) != RESET)
		{
			...
			uwb_size = 400 - __HAL_DMA_GET_COUNTER(&hdma_usart2_rx);
	...
	if(__HAL_UART_GET_FLAG(&huart6, UART_FLAG_IDLE) != RESET)
		{
			...
			data_size = 200 - __HAL_DMA_GET_COUNTER(&hdma_usart6_rx);
