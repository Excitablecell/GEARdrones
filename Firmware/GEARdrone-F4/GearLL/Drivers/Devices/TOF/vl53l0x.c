...
//	GPIO_InitTypeDef GPIO_Initure;
  ...
  if(Status!=VL53L0X_ERROR_NONE) goto error;
	...
	Status = VL53L0X_DataInit(pMyDevice);//设备Data初始化
	if(Status!=VL53L0X_ERROR_NONE) goto error;
	
	Status = VL53L0X_StaticInit(pMyDevice);//设备Static初始化
  if(Status!=VL53L0X_ERROR_NONE) goto error;
	
	Status=VL53L0X_PerformRefSpadManagement(pMyDevice, &refSpadCount, &isApertureSpads); //参考SPADs校准
	if(Status!=VL53L0X_ERROR_NONE) goto error;
	
	Status=VL53L0X_PerformRefCalibration(dev, &VhvSettings, &PhaseCal);            //温度校准
	if(Status!=VL53L0X_ERROR_NONE) goto error;
	
	Status = VL53L0X_SetReferenceSpads(dev,refSpadCount,isApertureSpads);   //设定参考SPADs的校准值
	if(Status!=VL53L0X_ERROR_NONE) goto error;
	
	Status=VL53L0X_SetRefCalibration(dev,VhvSettings,PhaseCal);             //设定温度校准值
	if(Status!=VL53L0X_ERROR_NONE) goto error;
