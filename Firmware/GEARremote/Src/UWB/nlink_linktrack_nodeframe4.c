...
typedef struct
  ...
  int  nlt_nodeframe4_raw_tag_size(const nlt_nodeframe4_raw_tag_t *data){
    return sizeof(*data) -sizeof(data->anchors)+ sizeof(data->anchors[0]) * data->anchor_count;
  }
...
static uint8_t UnpackData(const uint8_t *data, size_t data_length)
{
  ...
  {
    const nlt_nodeframe4_raw_tag_t *raw_tag = (const nlt_nodeframe4_raw_tag_t *)&data[addr];
    ...
    for (int j = 0; j < raw_tag->anchor_count; ++j)
    {
      ...
      tag->anchors[j]->dis = NLINK_ParseInt24(raw_tag->anchors[j].dis)/MULTIPLY_DIS;
    }
